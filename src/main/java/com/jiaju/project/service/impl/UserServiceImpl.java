...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    public boolean isRoot(HttpServletRequest request) {
        Object userObj = request.getSession().getAttribute(UserConstant.USER_LOGIN_STATE);
        User user = (User) userObj;
        return user != null && UserConstant.ROOT_ROLE.equals(user.getUserRole());
    }
    ...
    public boolean updateUser(User user) {
        ...
        if (StringUtils.isNotBlank(user.getWorkGroup())) {
            ...
            queryWrapper.eq("name", user.getWorkGroup());
            WorkGroup workGroup = workGroupMapper.selectOne(queryWrapper);
            if (workGroup == null) {
                throw new BusinessException(ErrorCode.PARAMS_ERROR, "工作组不存在");
            }
        }
